...
public class DriftController : MonoBehaviour
{
    ...
    {
        if (inBoost)
            accel *= Boost; // Higher acceleration
        Debug.Log(inThrottle);
    ...
    void RotateGradAbsolute(float angle)
    {
        ...
        if (delta * isCW < 0f)
            rot.y = angle; // Check if changed polarity
        else
            isRotating = true;
    ...
    void RotateGradRelative(float angle)
    {
        ...
        if (delta * isCW < 0f)
            drot.y = 0; // Check if changed polarity
        else
            isRotating = true;
